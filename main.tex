...
\texttt{adaptive pass fail} specifies an adaptive test where the tester clicks pass or fail for each trial. Targets are selected randomly with replacement but without consecutive selections.
...
\texttt{adaptive CRM} specifies an adaptive test where the subject responds using a colored number grid for each trial. Targets are selected randomly with replacement but without consecutive selections.
...
\section{Output}
The results of the test are written to a plaintext file located under \texttt{\mytilde/Documents/AVCoordinateResponseMeasureResults}. The name of the file is like \texttt{Subject\_wilfred\_Session\_123\_Experimenter\_456\_2018-6-24-10-30-0.txt}. The output file contains test settings and a record of trial-by-trial information. Listing~\ref{lst:example-output-file} shows such an output file.
